...
Other parents share **one Hosting URL** on a **separate Firebase project** (`recompenses-test` → https://recompenses-test.web.app). Login decides which family you see. Each family is `families/{familyId}/` (settings, tasks, billing, users). They never log into Anthony's Florent/Harry instance.
...
Do **not** create a new Firebase project per paying family. That is the old single-tenant trap (`billing/current` one owner, second parent blocked). This project **is** the 100-family platform.
...
1. **One Firebase project** for all paying families (`recompenses-test`, not `systemederecompensesenfants`).
2. `replica/public/js/firebase-config.js` — web app config for that project (repo keeps placeholders).
