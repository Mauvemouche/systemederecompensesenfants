...
window.applyReplicaFamily = function applyReplicaFamily(people) {
  const list = Array.isArray(people) ? people : [];
  PEOPLE = list.map((p) => p.id);
  CHILDREN = new Set(list.filter((p) => p.role === "child").map((p) => p.id));
};

window.startFamilyBoard = function startFamilyBoard() {
  if (window.__replicaState?.people) {
    applyReplicaFamily(window.__replicaState.people);
  }
  if (boardStarted) {
    loadTasks();
    return;
...
if (window.__replicaState?.hasAccess && !window.__replicaState?.needsKids) {
